...
namespace PRDCT.Core.PRDCTPeriodicity
{
    public class PeriodicitySunTerrain : Periodicity
    {
        ...
        {
            this.DataIvals = new List<Ivals>();
        }

        private double h_sun_min = 0.0 * MyMath.DegreesToRadians;
        ...
        public void Create()
        {
            ...
            foreach (var ival in base.DataIvals)
            {
                ...
                else if (isLeft == false && isRight == false)
                {
                    ...
                    this.DataIvals.Add(FuncSun2(ival, lonCur, tCur));
                }
                // интервал освещён частично
                //if( isLighting1 == false || isLighting2 == false )
                else
                {
                    this.DataIvals.Add(FuncSun1(ival, isLeft, isRight));
                }
